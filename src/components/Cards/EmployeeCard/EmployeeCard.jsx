...
const EmployeeCard = ({data}) => {
  ...
  return (
    <Card variant="cardBackground" className={card__container} sx={{borderRadius: "12px"}}>
      <Container sx={{padding: "0px", position: "relative", height: "260px", overflow: "hidden", borderRadius: "6px"}}>
        <Image src={data.img} fill sizes="100vh" alt="employee" style={{ width: "100%", height: "100%",objectFit: "cover"}}/>
      </Container>
      <Typography component="h3" variant='h3' className="h3" sx={{fontSize: 24, marginTop: "12px"}}>{data.nombre}</Typography>
      <Typography component="h4" variant='subtitleCard' className='h4' sx={{fontSize: 20,marginTop: "8px", textAlign: "center"}}>{data.cargo} - {data.sucursal}</Typography>
